...
class OssUtil
{
    const GET_IMAGE_INFO_SUFFIX = '?x-oss-process=image/info';
    ...
    {
        $inputFileName = urldecode($input_file['Object']);
        $inputFileName = substr($inputFileName, 0, strrpos($inputFileName, '.'));
        $obj           = $inputFileName . '.jpg';
    ...
    /**
     * Get the oss image's info.
     * Such as width,height.
     * @return \stdClass
     */
    public function getImageInfo($ossImageUrl)
    {
        if (empty($ossImageUrl)) {
            return null;
        }
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $ossImageUrl . static::GET_IMAGE_INFO_SUFFIX);
        ...
        $imgInfo->file_size = $result->FileSize->value;
        $imgInfo->type      = $result->Format->value;
        $imgInfo->width     = $result->ImageWidth->value;
        $imgInfo->height    = $result->ImageHeight->value;

        return $imgInfo;
    }

    /**
     * Get pictures with size info, for ios detail page
     * image autofit.
     *
     ...
     * @return array
     */
    public static function getPicturesWithSizeInfo(array $pictures)
    {
        $ossUtil = new static;
        return array_map(function ($ossImageurl) use ($ossUtil) {
            $imageSizeInfo = $ossUtil->getImageInfo($ossImageurl);
            return [
                'w'   => $imageSizeInfo->width,
                'h'   => $imageSizeInfo->height,
                'url' => $ossImageurl
            ];
        }, $pictures);
    }
