...
class Inspire extends Command
{
    ...
    public function handle()
    {
        foreach (User::all() as $user) {
            ...
            if (!$profile || !$profile->name || ($user->FNAME == $profile->name)) {
                $this->warn('user '.$user->FID.'not have profile name');
                continue;
            }
            User::where('FID', $user->FID)->update(['FNAME' => $profile->name]);
            $this->info('user '.$user->FID.'   '.$user->FNAME.'-->'.$profile->name);
        }
    }
