...
trait FriendOperationTrait
{
    ...
    public function deleteFriend($friendId)
    {
        ...
        foreach ($friendShips as $friendShip) {
            ...
            //删除用户收到的
            $user->receivedApplications()->where('applier_id', $friendId)->delete();
    ...
    {
        Friend::create(['user_id' => $this->FID, 'friend_id' => $friendUser->FID,]);

        Friend::create(['user_id' => $this->FID, 'friend_id' => $friendUser->FID,]);

        //添加环信好友因为删除是双线的,所以加好友也是双向的
        $this->addHxFriend($friendUser->FID);
        $friendUser->addHxFriend($this->FID);
    }
