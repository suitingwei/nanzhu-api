...
trait GroupOperationTrait
{
    ...
    public function tryJoinMovieGroup($groupIds, $movieId)
    {
        ...
        if ($this->hadNotTryJoinedGroup($groupIds)) {
            return JoinGroup::createNewJoin($this->FID, $groupIds, $movieId);
        }
    ...
    {
        (new ExitBehaviorManager($this))->exitGroup($groupId);
    }
    ...
    public function updateGroupInfo($newData)
    {
        ...
        foreach ($newData['phoneJson'] as $phoneInfo) {
            ...
            if ($phoneInfo) {
                SparePhone::find($phoneInfo->spare_phone_id ?: $phoneInfo->share_phone_id)->update([
     ...
     * @return GroupUser
     */
    public function joinGroup(Group $group, $position = '', $isAdmin = false)
    {
        return GroupUser::create([
            ...
            'FREMARK'        => $position,
            'FGROUPUSERROLE' => $isAdmin ? Movie::ROLE_ADMIN : Movie::ROLE_COMMON_USER,
            'FOPEN'          => 10,
            'FOPENED'        => 1,
            'FPUBLICTEL'     => 20,
