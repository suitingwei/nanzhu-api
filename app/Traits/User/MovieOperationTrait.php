...
use App\Managers\Behaviors\ExitBehaviorManager;
...
trait MovieOperationTrait
{
    ...
    {
        (new ExitBehaviorManager($this))->exitMovie($movieId);
    }
    ...
    /**
     * Judge whether a user is changji in a movie.
     *
     * @param $movieId
     *
     * @return boolean
     */
    public function isChangJiInMovie($movieId)
    {
        return GroupUser::is_changji($movieId, $this->FID);
    }
    ...
    {
        $movieId = GroupUser::where('FUSER', $this->FID)->selectRaw('distinct FMOVIE')->lists('FMOVIE')->all();
     ...
     * @return Collection
     */
    public function joinedNotEndMovies()
    {
        $movieId = GroupUser::where('FUSER', $this->FID)->selectRaw('distinct FMOVIE')->lists('FMOVIE')->all();

        return Movie::whereIn('FID', $movieId)->notEnd()->get();
    }
