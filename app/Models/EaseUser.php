...
class EaseUser extends Model
{
    ...
    public function getGroupInfo($hxGroupId, $currentUser)
    {
        ...
        $result = [
            ...
            'title'         => $group ? $group->hx_title : '未命名群聊',
    ...
    public function groupAddUser($hxGroupId, $userId)
    {
        try {
            $userName = self::USER_NAME_PREFIX . $userId;
            $result   = $this->easemob->groupAddUser($hxGroupId, $userName);
        } catch (\Exception $e) {
    ...
    private function getPublicNotice($hxGroupId)
    {
        ...
        if ($group) {
            $publicNotice = $group->publicNotices()->orderBy('created_at', 'desc')->first();
        }
        else {
            $customHxGroup = CustomHxGroup::where('hx_group_id', $hxGroupId)->first();
            if ($customHxGroup) {
                $publicNotice = $customHxGroup->publicNotices()->orderBy('created_at', 'desc')->first();
            }
        }
