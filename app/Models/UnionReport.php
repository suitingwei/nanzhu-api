 ...
 */
class UnionReport extends Model implements ReadStatus
{
    //某一个用户读取场记日报表
    ...
    public function pushMessages()
    {
        ...
        $message->push(true, ['union_id' => $this->id]);
    }
    ...
    private function createPushMessage()
    {
        ...
        if (!$this->authorUser->hadAssignedPowerInMovie($this->movie_id, UnionPower::class)) {
            UnionPower::assignUser($this->authorUser, $this->movie_id);
        }

        $allUsersWithUnionPower = $this->movie->allUsersWithPower(UnionPower::class)->pluck('FID')->all();

        return Message::create([
            'type'            => Message::TYPE_DAILY_REPORT,
            'scope'           => 1,
            "scope_ids"       => implode(',', $allUsersWithUnionPower),
