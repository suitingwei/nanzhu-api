...
class Profile extends Model
{
    ...
    public function toArray()
    {
        return [
            ...
            'phone'                      => $this->user ? $this->user->FPHONE : 0
        ];
    }
    ...
    /*protected static function boot()
    {
        ...
        static::saved(function($profile){
            \Log::info('profile'.$profile->before_position);
            if($profile->before_position!=""){
                $beforeArr=explode(',',$profile->before_position);

                if($beforeArr[0]!='男' && $beforeArr[0]!='女'){
                foreach ($beforeArr as $key => $value) {
                    if ($value == '男') {
                        unset($beforeArr[$key]);
                        array_unshift($beforeArr, '男');
                    }
                    if ($value == '女') {
                        unset($beforeArr[$key]);
                        array_unshift($beforeArr, '女');
                    }
                }
                $beforeArr=implode(',',$beforeArr);
                \Log::info('profile'.$beforeArr);
                $profile->update(['behind_position',$beforeArr]);
                }
            }
            if($profile->behind_position!=""){
                $behindArr=explode(',',$profile->behind_position);
                if($behindArr[0]!='男' && $behindArr[0]!='女') {
                    foreach ($behindArr as $key => $value) {
                        if ($value == '男') {
                            unset($behindArr[$key]);
                            array_unshift($behindArr, '男');
                        }
                        if ($value == '女') {
                            unset($behindArr[$key]);
                            array_unshift($behindArr, '女');
                        }
                    }
                    $behindArr = implode(',', $behindArr);
                    \Log::info('profile' . $behindArr);
                    $profile->update(['behind_position', $behindArr]);
                }
            }
            \Log::info("show this profile" . $profile);
