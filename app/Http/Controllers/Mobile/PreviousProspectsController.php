...
class PreviousProspectsController extends BaseController
{
    ...
    public function receivers($dailyReportId, Request $request)
    {
        $receivers    = [];
        $un_receivers = [];
        $dailyReport  = PreviousProspect::find($dailyReportId);
        ...
        return view("mobile.previous_prospects.receivers", ["receivers" => $receivers, "un_receivers" => $un_receivers, 'movieId' => $request->input('movie_id')]);
    }
}
