...
module.exports = React.createClass({
    ...
    render: function () {
        ...
        for (var i = 0; i < this.state.friends.length; i++) {
            userArray.push(this.state.friends[i].name.replace('nanzhu_', ''));
        }
        ...
        if (userIds != '') {
            $.get('api/im/getUsersInfo?user_ids=' + userIds, function (responseData) {
                ...
                for (var i = 0; i < self.state.friends.length; i++) {
                    var userInfo = usersInfo[i];
                    self.state.friends[i].nickname = userInfo.user_name;
                }
            });
        }
        ...
        for (var i = 0; i < this.state.friends.length; i++) {
            ...
            props.name = id;
            props.nickname = this.state.friends[i].nickname ? this.state.friends[i].nickname : '';
        ...
        for (var i = 0; i < this.state.groups.length; i++) {
            groupArray.push(this.state.groups[i].roomId);
        }
        ...
        if (groupIds != '') {
            $.get('api/im/getGroupsInfo?group_ids=' + groupIds, function (responseData) {
                ...
                for (var i = 0; i < self.state.groups.length; i++) {
                    self.state.groups[i].nickname = groupsInfo[i];
                }
            });
        }
        ...
        for (var i = 0; i < this.state.groups.length; i++) {
            ...
            props.nickname = this.state.groups[i].nickname ? this.state.groups[i].nickname : '';
            if(props.nickname !=''){
