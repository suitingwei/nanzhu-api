...
<footer class="mui-bar mui-bar-footer bar-footer2">
    ...
    <button id="done" type="button" class="mui-btn mui-btn-success mui-pull-right">完成</button>
</footer><!--/end-->
...
<div class="contact ui-content">
    <div id='list' class="mui-indexed-list">
        ...
        <div class="mui-indexed-list-inner">
            ...
            <form id="addPublicForm" action="/mobile/users/public_contact/" method="post">
                ...
                <ul class="mui-table-view">
                    @foreach($groupedUsers as $groupKey => $users)
                        ...
                        @foreach($users as $user)
                            <li data-value="ALM" data-tags="{{ $user->FNAME }}" class="active mui-table-view-cell mui-media mui-checkbox mui-indexed-list-item">
                                <input class="active-op" name="subBox" type="checkbox" user-id="{{ $user->FID }}"
                                    @if($user->hadJoinedPublicContactsInMovie($movieId))
                                    checked
                                    @endif>
                                <input type="hidden" name="publicContact[]">
                                <a class="active-info">
                                    <span class="pic"><img class="mui-media-object mui-pull-left" src="{{ $user->cover_url}}"></span>
