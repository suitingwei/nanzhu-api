...
<div class="details-resource">
    @foreach($messages  as  $message)
        <a href="/mobile/unions/detail/UNIONCOOPRAT/{{$unionId}}/{{$message->id}}?user_id={{$user_id}}&title={{$message->title}}">
            <?php $is_read=DB::table('message_receivers')->where('receiver_id',$user_id)->where('message_id',$message->id)->value('is_read');
            $url    =DB::table('pictures')->where("message_id",$message->id)->orderBy('updated_at','desc')->first();
            ?>
            <div class="resource-bw">
                @if($is_read==0)
                    <img src="http://nanzhu.oss-cn-shanghai.aliyuncs.com/unions/new@2x.png" class="readornot">
                @endif
...
<a class="btn mui-btn-success" href="/mobile/unions/apply/{{$unionId}}?user_id={{$user_id}}&title=申请提交" style="position: fixed;bottom:0;height:50px;line-height: 50px ;text-align: center;width: 100%;">提交申请</a>
...
<script>
    ...
    $(document).ready(function () {
        window.nanzhu.showTitle('联盟项目合作',false,{});
    })
